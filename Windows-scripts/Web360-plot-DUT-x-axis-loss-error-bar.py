#plot mean DUT and bytes for Web360
# where there are 3 images in each run and 3 tasks: drag-and-drop, zoom-in, and zoom-out
...
def Compute (run_no,task_i):
    ...
    for meth in method:
        ...
        #read rt
        rt_meth = "rt_"+meth
        globals()[rt_meth] = globals()[data_meth][:,np.arange(2+task_i,10,3)] #read specific col. of rt, remember the 1st two are rtt and loss so skip

        #read bytes
        by_meth = "by_"+meth
        globals()[by_meth] = globals()[data_meth][:,np.arange(2+9+task_i,9+10,3)]
...
markers = ['^','s','o','*','x','D','+']
leg_title = ["drag-and-drop","zoom-in","zoom-out"]
...
for app in App:
    for i in range(0,3):
        rt,by,rt_error,by_error = Compute(Run_no[0],i)
        ax1.errorbar(loss_uniq,rt,color=colors[i],yerr=rt_error,marker=markers[i],linewidth=2.0,markersize=10,label = 'DUT '+leg_title[i])
#    i+=1
...
for app in App:
    for i in range(0,3):
        rt,by,rt_error,by_error = Compute(Run_no[0],i)
        ax2.errorbar(loss_uniq,by,color=colors[i],yerr=by_error,marker=markers[i],linewidth=2.0,markersize=10,linestyle='dashed',label ='MB '+leg_title[i])
#    i+=1
